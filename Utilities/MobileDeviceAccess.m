...
id				AMDeviceSetValue(am_device device,CFStringRef domain,CFStringRef key,CFStringRef value);
...
- (bool)writeN:(afc_long)n bytes:(const char *)buff
{
	if (![self ensureFileIsOpen]) return NO;
	if (n>0) {
		return [self checkStatus:AFCFileRefWrite(_afc, _ref, buff, (uint32_t)n) from:"AFCFileRefWrite"];
	}
	return YES;
}
...
- (id)setDeviceName:(NSString *)name
{
	return [self setDeviceValue:name forKey:@"DeviceName"];
}

- (id)setDeviceValue:(NSString *)value forKey:(NSString*)key
{
	return [self setDeviceValue:value forKey:key inDomain:nil];
}

- (id)setDeviceValue:(NSString *)value forKey:(NSString*)key inDomain:(NSString*)domain
{
	BOOL opened_connection = NO;
	BOOL opened_session = NO;
	id result = nil;
	...
	// ok we have a session running, just query and set up to return
	result = (id)AMDeviceSetValue(_device,(CFStringRef)domain,(CFStringRef)key,(CFStringRef)value);
